...
import HowToVideoCard from '../../components/VideoCard/HowToVideoCard';
...
class HowToVideo extends React.Component {
    ...
    render() {
        const { search, howToVideoData, Loading } = this.state;
        if (Loading === false) {
            console.log(howToVideoData);
        }
        return (
            <View style={styles.container}>
                <NavBarDefault name={'How To Videos'} onPress={() => this.proceedBack()} />
                <ScrollView>
                            ...
                            {Loading ? (
                                <ActivityIndicator size="large" color="#0000ff" />
                                    ...
                                    howToVideoData.map((videos, i) => (
                                        <HowToVideoCard key={i} videos={videos} onPress={() => this.proceedPlayer()} />
                                    ))
                                )
                            }
                        </View>
